...
async function loadDashboardStats(page, storeId) {
  try {
    ...
    // Update stats cards
    page.querySelector('#total-products').textContent = performance.total_products || 0
    page.querySelector('#total-revenue').textContent = `$${(analytics.total_revenue || 0).toLocaleString()}`
    page.querySelector('#total-orders').textContent = analytics.total_orders || 0
    page.querySelector('#avg-rating').textContent = (performance.average_rating || 0).toFixed(1)
          ...
          <tbody>
            ${recentProducts.map(product => `
              <tr class="border-b hover:bg-gray-50">
                ...
                <td class="p-3 font-medium">$${product.final_price}</td>
                <td class="p-3">
  ...
  try {
    ...
    chartContainer.innerHTML = `
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div class="text-center">
          <div class="text-2xl font-bold text-secondary mb-2">${analytics.total_views || 0}</div>
          <p class="text-sm text-muted">Total Views</p>
        </div>
        <div class="text-center">
          <div class="text-2xl font-bold text-primary mb-2">${analytics.unique_visitors || 0}</div>
          <p class="text-sm text-muted">Unique Visitors</p>
        </div>
        <div class="text-center">
          <div class="text-2xl font-bold text-success mb-2">${(analytics.conversion_rate || 0).toFixed(2)}%</div>
      ...
      <div class="mt-6 p-4 bg-gray-50 rounded">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
          <div>
            <p class="text-muted">Product Views: <span class="font-medium">${analytics.product_views || 0}</span></p>
            <p class="text-muted">Add to Cart: <span class="font-medium">${analytics.add_to_cart_count || 0}</span></p>
          </div>
          <div>
            <p class="text-muted">Average Order Value: <span class="font-medium">$${(analytics.average_order_value || 0).toFixed(2)}</span></p>
            <p class="text-muted">Total Clicks: <span class="font-medium">${analytics.total_clicks || 0}</span></p>
          </div>
        </div>
      </div>
    `
